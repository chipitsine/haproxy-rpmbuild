...
%{__make} %{?_smp_mflags} CPU="generic" TARGET="linux-glibc" USE_SYSTEMD=1 USE_OPENSSL=1 USE_PCRE=1 USE_ZLIB=1 USE_LUA=1 LUA_LIB=/usr/local/lib LUA_INC=/usr/local/include ${regparm_opts} ADDINC="%{optflags}" ADDLIB="%{__global_ldflags}" DEFINE=-DTCP_USER_TIMEOUT=18
